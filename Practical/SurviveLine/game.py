...
class Game:
    ...
    def displayScore(self):
        yPos = 40
        text = Game.SCORE_FONT.render(
            "{0}".format(self.Wave.ScoreCount // 200), True, DATA_FONT_COLOR
        )
    ...
    def displayAINum(self, genNum, genomeNum):
        yPos = 70
        text = Game.NORMAL_FONT.render(
            "{0}.{1}".format(genNum, genomeNum), True, DATA_FONT_COLOR
        )
        text_rect = text.get_rect(center=(self.WDisplay / 2, yPos))
        self.window.blit(text, text_rect)
    ...
    def collision(self, runLoop):
        ...
        for YCord in range(self.HDisplay - ball.bottom, self.HDisplay - ball.top):
            if (Wave.PointsList[YCord] != 0) and (
                ball.right >= Wave.PointsList[YCord] - 50 - Wave.WaveGap
            ):
                return runLoop == False

            if (Wave.PointsList[YCord] != 0) and ball.left <= Wave.PointsList[
                YCord
            ] - 350 + Wave.WaveGap:
                return runLoop == False
    ...
    def moveBall(self, dir):
        if (
            dir == "Right"
            and self.Ball.ballCordX + (Ball.BALL_RADIUS * 2) < self.WDisplay
        ):
    ...
    def countDistance(self):
        ...
        for YCord in range(
            self.HDisplay - ball.bottom, self.HDisplay - ball.top + 20, 5
        ):
            for YBall in range(0, ball.width, 5):
                dxR = pow(
                    Wave.PointsList[YCord]
                    - 50
                    - Wave.WaveGap
                    - ball.bottomright[0]
                    + YBall,
    ...
    def showVision(self):
        ...
        for disLine in range(len(self.showedLines)):
            ...
            if self.Wave.PointsList[xCord] != 0:
                pygame.draw.line(
                    self.Ball.GameDisplay,
                    lineToWaveColour,
                    (ball.centerx, ball.centery),
                    (self.Wave.PointsList[xCord] - 50 - self.Wave.WaveGap, 800 - xCord),
                    2,
                )
                pygame.draw.line(
                    self.Ball.GameDisplay,
                    ...
                    (ball.centerx, ball.centery),
                    (
                        self.Wave.PointsList[xCord] - 350 + self.Wave.WaveGap,
                        800 - xCord,
                    ),
                    2,
                )
