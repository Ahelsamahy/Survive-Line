...
posX = cycle(range(2))
pygame.init()
GAME_DISPLAY = pygame.display.set_mode((DISPLAY_W, DISPLAY_H), pygame.NOFRAME) # Make a window to show on
...
# def showWave():
#         for show in range(DISPLAY_H):
#             pygame.gfxdraw.pixel(GAME_DISPLAY, POINTS_MATRIX[CORD_Y][next(posX)]-55, POINTS_MATRIX[CORD_Y][next(posX)]+show,(255, 255, 255))
...
def run_game():
    ...
    while running:
        ...
        for Y_CORD in range(500):
            pygame.gfxdraw.pixel(GAME_DISPLAY, POINTS_MATRIX[Y_CORD][next(posX)]-55, POINTS_MATRIX[Y_CORD][next(posX)]+SCORE_COUNTER,(255, 255, 255))
        pygame.display.flip()

        # for show in range(0,DISPLAY_H):
        #     pygame.gfxdraw.pixel(GAME_DISPLAY, POINTS_MATRIX[y][next(posX)]-55, POINTS_MATRIX[y][next(posX)]+show, (255, 255, 255))
